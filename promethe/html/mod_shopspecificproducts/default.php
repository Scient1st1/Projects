...
?>
<div class="shop_products shop_products_home">
	<div class="row">
		<?php
		$c = 1;
		$k = 0;
		foreach ( $modarticles->list as $item )
		{
			...
			<div class="catalog_item col-md-3 col-sm-6 col-xs-12">
				...
				<div class="catalog_item_ini ">
					...
					{
						?>
						<?php $mainImg = GoodWebSiteHelper::getImageInSize( 'category', C::_( 'main_image', $item ) ); ?>
	...
	<?php } ?>
							<a href="<?= $productLink ?>">
                                                            <div class="ItemImagein"></div>
							<img class="classifier_img" src="<?= $mainImg ?>" >
		<?php if ( $hoverImg ): ?>
			<img class="classifier_imgh" src="<?= $hoverImg ?>" > 
		<?php endif; ?>
							</a> 
							<?php if(C::_( 'sale_price', $item )){ echo '<div class="saleicon">'.JText::_('Sale').'</div>'; } ?>   
						</div>
					<?php
					}
